...
def sample_mazes(G,
    ...
    if not from_dataset:
        maze_gen = maze_generator.Maze(G)
    else:
        maze_gen = [
            from_dataset + "/" + x for x in os.listdir(from_dataset)
            if x[-3:] == "pbm"
        ]
        np.random.shuffle(maze_gen)

    assert len(
        maze_gen
    ) >= num_samples, f"not enough data ({len(maze_gen)} but {num_samples} was requested)"
...
class HardMaze(Problem):
    ...
    def __call__(self, ag):
        ...
        for i in range(self.max_steps):
            ...
            if self.bd_type == "generic":
                ...
                behavior_info = cv2.circle(behavior_info,
                                           (int(z[0]), int(z[1])),
                                           2, (255, 0, 0),
                                           thickness=-1)
        ...
        dist_to_goal = np.linalg.norm(
            np.array(info["robot_pos"][:2]) -
            np.array([self.env.goal.get_x(),
                      self.env.goal.get_y()]))
    ...
    def visualise_bds(self,
        ...
        if quitely and not (len(save_to)):
            raise Exception(
                "quitely=True requires save_to to be an existing directory")
        #quitely=False
        ...
        for pt_i in range(z.shape[0]):
            ...
            #    #color=MiscUtils.colors.blue
            #    thickness=1
            maze_im = cv2.circle(maze_im, (int(z[pt_i, 0]), int(z[pt_i, 1])),
                                 3,
                                 color=color,
                                 thickness=thickness)
        ...
        maze_im = cv2.circle(
            maze_im, (int(goal.get_x() * self.maze_im.shape[0] / real_h),
                      int(goal.get_y() * self.maze_im.shape[1] / real_w)),
            3, (0, 0, 0),
            thickness=-1)
        maze_im = cv2.circle(
            maze_im, (int(goal.get_x() * self.maze_im.shape[0] / real_h),
                      int(goal.get_y() * self.maze_im.shape[1] / real_w)),
            int(self.goal_radius * self.maze_im.shape[0] / real_h), (0, 0, 0),
            thickness=1)
        ...
        if not quitely:
            ...
            if len(save_to):
                ...
                cv2.imwrite(
                    save_to + "/hardmaze_2d_bd_" + str(gen_num) + ".png",
                    maze_im)
                self.num_saved += 1

...
class Maze:
    ...
    def enable_all_walls_and_fill(self):
        ...
        for i in range(self.N):  #iterating over the diagonal
            for j in range(1, self.N):
                ...
                self.set_wall_between_two_cells(0, i, j - 1, i, j,
                                                fill=True)  #vertical walls
                self.set_wall_between_two_cells(0, j - 1, i, j, i,
                                                fill=True)  #horizontal walls
    ...
    def set_wall_between_two_cells(self, val, a_x, a_y, b_x, b_y, fill=False):
        ...
        if d_x == 1 and d_y == 0:
            ...
            wall_lims_y = [
                b_y * self.B_s + b_y * self.W_s,
                (b_y + 1) * self.B_s + b_y * self.W_s
            ]

            self.mz[wall_lims_x[0]:wall_lims_x[1],
                    wall_lims_y[0]:wall_lims_y[1]] = val

            if fill and b_y != self.N - 1:
                self.mz[wall_lims_x[0]:wall_lims_x[1],
                        wall_lims_y[1]:wall_lims_y[1] + self.W_s] = val
        ...
        if d_y == 1 and d_x == 0:
            ...
            wall_lims_x = [
                b_x * self.B_s + b_x * self.W_s,
                (b_x + 1) * self.B_s + b_x * self.W_s
            ]
            ...
            if fill and b_x != self.N - 1:
                self.mz[wall_lims_x[1]:wall_lims_x[1] + self.W_s,
                        wall_lims_y[0]:wall_lims_y[1]] = val
